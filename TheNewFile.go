...
// NewUndoBuffer initializes a new buffer with the given content as a starting point.
// To start with an empty buffer pass nil as a content.
func NewUndoBuffer(content RuneArray) *Buffer {
...
func (b *Buffer) Insert(off int64, data RuneArray) error {
	if len(data) == 0 {
		return nil
	}
...
func (b *Buffer) Delete(off, length int64) error {
	...
	if cur == length {
		...
		newBuf := make(RuneArray, len(p.data[beg:]))
	...
	if midwayStart {
		...
		newBuf := make(RuneArray, len(start.data[:offset]))
...
func (b *Buffer) newPiece(data RuneArray, prev, next *piece) *piece {
	b.piecesCnt++
	return &piece{
		id:   b.piecesCnt,
		prev: prev,
...
func (b *Buffer) ReadAt(data RuneArray, off int64) (n int, err error) {
	p := b.begin
	for ; p != nil; p = p.next {
		if off < int64(p.len()) {
			break
...
func (p *piece) insert(off int, data RuneArray) {
