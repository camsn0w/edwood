...
func printarg(argt *Text, q0 int, q1 int) string {
	if argt.what != Body || argt.oeb.Name() == "" {
		return ""
	}
	if q0 == q1 {
		return fmt.Sprintf("%s:#%d", argt.oeb.Name(), q0)
	}
	return fmt.Sprintf("%s:#%d,#%d", argt.oeb.Name(), q0, q1)
}
...
func cut(et *Text, t *Text, _ *Text, dosnarf bool, docut bool, _ string) {
	...
	if et != t && dosnarf && et.w != nil {
		if et.w.body.q1 > et.w.body.q0 {
			t = &et.w.body
			if docut {
				t.oeb.Mark(seq) // seq has been incremented by execute
			}
		} else {
...
func tab(et *Text, _ *Text, argt *Text, _, _ bool, arg string) {
	...
	if tab > 0 {
		...
		warning(nil, "%s: Tab %d\n", w.body.oeb.Name(), w.body.tabstop)
	}
}
