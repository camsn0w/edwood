...
func TestExpand(t *testing.T) {
	...
	for i, tc := range tt {
		t.Run(fmt.Sprintf("test-%02d", i), func(t *testing.T) {
			r := []rune(tc.s)
			text := &Text{
				oeb: file.MakeObservableEditableBufferTag(file.RuneArray(r)),
				q0:  0,
				q1:  tc.sel1,
			}
...
func TestExpandJump(t *testing.T) {
	...
	for _, tc := range tt {
		text := &Text{
			oeb:  file.MakeObservableEditableBufferTag([]rune("chicken")),
			q0:   0,
			q1:   5,
			what: tc.kind,
		}
...
func textSetSelection(t *Text, buf string) {
	...
	t.q0 = popRune('«')
	t.q1 = popRune('»')
	t.oeb = file.MakeObservableEditableBuffer("", b)
}
