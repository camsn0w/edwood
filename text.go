...
func (t *Text) Redraw(r image.Rectangle, odx int, noredraw bool) {
	...
	if t.what == Body && t.oeb.IsDir() && odx != t.all.Dx() {
...
func (t *Text) Columnate(names []string, widths []int) {
	...
	for i := 0; i < nrow; i++ {
		...
		t.oeb.InsertAt(q1, Lnl)
		q1++
	}
}

func (t *Text) checkSafeToLoad(filename string) error {
	if t.oeb.HasUncommitedChanges() || t.oeb.FileSize() > 0 || t.w == nil || t != &t.w.body {
		panic("editor.load")
	}

	if t.oeb.IsDir() && t.oeb.Name() == "" {
...
func (t *Text) loadReader(q0 int, filename string, rd io.Reader, sethash bool) (nread int, err error) {
	t.oeb.SetDir(false)
	t.w.filemenu = true
	count, hasNulls, err := t.oeb.Load(q0, rd, sethash)
	if err != nil {
...
func (t *Text) Load(q0 int, filename string, setqid bool) (nread int, err error) {
	...
	if d.IsDir() {
		...
		t.oeb.SetDir(true)
		t.w.filemenu = false
		if len(t.oeb.Name()) > 0 && !strings.HasSuffix(t.oeb.Name(), string(filepath.Separator)) {
			t.oeb.SetName(t.oeb.Name() + string(filepath.Separator))
			t.w.SetName(t.oeb.Name())
		}
...
func (t *Text) Insert(q0 int, r []rune, tofile bool) {
	...
	if tofile && t.oeb.HasUncommitedChanges() {
		panic("editor.insert")
	}
	if len(r) == 0 {
		return
...
func (t *Text) fill(fr frame.SelectScrollUpdater) error {
	...
	if t.oeb.HasUncommitedChanges() {
		// TODO(rjk): This should probably be t.file.Commit()
		t.TypeCommit()
	}
	for {
		n := t.oeb.FileSize() - (t.org + fr.GetFrameFillStatus().Nchars)
		if n < 0 {
