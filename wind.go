...
func (w *Window) DrawButton() {
	b := button
	if w.body.oeb.SaveableAndDirty() {
		b = modbutton
	}
	var br image.Rectangle
	br.Min = w.tag.scrollr.Min
...
func (w *Window) Undo(isundo bool) {
	w.utflastqid = -1
	body := &w.body
	if q0, q1, ok := body.oeb.Undo(isundo); ok {
...
func (w *Window) setTag1() {
	...
	sb.WriteString(w.body.oeb.Name())
