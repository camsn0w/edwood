...
func TestXfidwriteQWaddr(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			w.body.file.SetB(RuneArray("abcαβξ\n"))
			w.col = new(Column)
...
func TestXfidwriteQWtag(t *testing.T) {
	...
	w.body.file = NewFile("")
	w.tag.file = NewFile("")
	w.tag.file.SetB(RuneArray(prevTag))
	x := &Xfid{
		fcall: plan9.Fcall{
			Data:  []byte(extra),
...
func TestXfidwriteQWerrors(t *testing.T) {
	...
	w := NewWindow().initHeadless(nil)
	w.col = new(Column)
	w.tag.file.SetB(RuneArray("/home/gopher/edwood/row.go Del Snarf | Look "))
	w.tag.fr = &MockFrame{}
	w.body.fr = &MockFrame{}
	x := &Xfid{
		fcall: plan9.Fcall{
...
func TestXfidreadQWbodyQWtag(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			switch tc.q {
			case QWbody:
				w.body.file.SetB(RuneArray(data))
			case QWtag:
				w.tag.file.SetB(RuneArray(data))
			}
...
func TestXfidreadQWxdataQWdata(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			w := NewWindow().initHeadless(nil)
			w.col = new(Column)
			w.body.file.SetB(RuneArray(body))
			w.addr = tc.inAddr
			xfidread(&Xfid{
				f: &Fid{
...
func TestXfidreadQWctl(t *testing.T) {
	...
	w.body.fr = &MockFrame{}
	w.tag.file.SetB(RuneArray("/etc/hosts Del Snarf | Look Get "))
	w.body.file.SetB(RuneArray("Hello, world!\n"))
